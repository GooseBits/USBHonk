...
##
## Replace the firstboot resize script with a custom one
## This expands the rootfs to 8GB. The rest is for user storage.
##

cat << 'FBEOF' > /usr/lib/raspi-config/init_resize.sh
#!/bin/sh
...
get_variables () {
  ...
  ROOT_DEV_BLK_SIZE=$(cat "/sys/block/${ROOT_DEV_NAME}/queue/logical_block_size")
  ROOT_PART_NUM=$(cat "/sys/block/${ROOT_DEV_NAME}/${ROOT_PART_NAME}/partition")
  ROOT_PART_START=$(cat "/sys/block/${ROOT_DEV_NAME}/${ROOT_PART_NAME}/start")
  ROOT_PART_TARGET_SECTORS=$((ROOT_PART_TARGET_SIZE / ROOT_DEV_BLK_SIZE))
